...
data class GetLocalDateTime(
    ...
    operator fun invoke(key: String, defaultValue: LocalDateTime): LocalDateTime {
        val rawDate = sharedPreferences.getString(key, "")
        return if (rawDate == "")
            defaultValue
        else
            LocalDateTime.parse(rawDate)
    ...
    operator fun invoke(@StringRes key: Int, defaultValue: LocalDateTime, vararg param: Int): LocalDateTime {
